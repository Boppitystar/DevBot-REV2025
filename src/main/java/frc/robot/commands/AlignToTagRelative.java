...
public class AlignToTagRelative extends Command {
  ...
  public void initialize() {
    ...
    mRotController.setSetpoint(Constants.AutoConstants.ROT_SETPOINT_REEF_ALIGNMENT);
    mRotController.setTolerance(Constants.AutoConstants.ROT_TOLERANCE_REEF_ALIGNMENT);

    mXController.setSetpoint(Constants.AutoConstants.X_SETPOINT_REEF_ALIGNMENT);
    mXController.setTolerance(Constants.AutoConstants.X_TOLERANCE_REEF_ALIGNMENT);

    mYController.setSetpoint(isRightScore ? Constants.AutoConstants.Y_SETPOINT_REEF_ALIGNMENT
    : -Constants.AutoConstants.Y_SETPOINT_REEF_ALIGNMENT);
    mYController.setTolerance(Constants.AutoConstants.Y_TOLERANCE_REEF_ALIGNMENT);
  ...
  public void execute() {
    if (LimelightHelpers.getTV("") && LimelightHelpers.getFiducialID("") == tagID) {
      ...
      double xSpeed = mXController.calculate(positions[2]);
      double ySpeed = -mYController.calculate(positions[0]);
      double rotValue = -mRotController.calculate(positions[4]); 
      
      SmartDashboard.putNumber("xspeed", xSpeed);

      mDrivebase.drive(xSpeed, ySpeed, rotValue, false);
  ...
  public void end(boolean interrupted) {
    mDrivebase.drive(0,0, 0, false); //test to see if true/change back to field relative works 
  }
